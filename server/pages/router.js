...
router.get("/register", (req, res) => {
  res.render("register", { user: req.user ? req.user : {}, loginUser: req.user, });
});

router.get("/new", async (req, res) => {
  const allGenres = await Genres.find();
  res.render("newBlog", { genres: allGenres, user: req.user ? req.user : {}, loginUser: req.user, });
});
...
router.get("/profile/:id", async (req, res) => {
  ...
  if (user) {
    const blog = await Blog.find({ author: user._id }).populate("genre").populate("author");
    res.render("profile", {
      user: user,
      genres: allGenres,
